...
//    				size = sprintf((char *)dbuf, "200 UTF8 set to on");
//    				send(CTRL_SOCK, dbuf, size);
    			break;					
					case NOOP_CMD:					
									...
									printf("NOOP_CMD\r\n");
						#endif			
//						slen = sprintf((char *)sendbuf, "200 NOOP command successful");
...
#endif
//#if !defined(F_FILESYSTEM)
					if ((strncmp(ftp.workingdir, "/", sizeof("/")) == 0)||(strncmp(ftp.workingdir, "", sizeof("")) == 0))
					{
						size = Network_FTP_Answer_MLSD_CMD(send,dbuf);
						ftp.current_cmd = NO_CMD;
    				disconnect(DATA_SOCK);
...
//#endif
					else if (strncmp(ftp.workingdir, "/$Recycle.Bin", sizeof("/$Recycle.Bin")) == 0)
					{
    				size = Network_FTP_Answer_Recycle_MLSD_CMD(send,dbuf);
    				ftp.current_cmd = NO_CMD;
    				disconnect(DATA_SOCK);
    				size = sprintf((char *)dbuf, "226 Successfully transferred \"%s\"\r\n", ftp.workingdir);
    				send(CTRL_SOCK, dbuf, size);
...
//    				disconnect(DATA_SOCK);
//    				size = sprintf((char *)dbuf, "502 Command does not implemented yet.\"%s\"\r\n", ftp.workingdir);
//    				send(CTRL_SOCK, dbuf, size);
...
//#else
//						
//						size=Network_FTP_Answer_RETR_CMD(send, dbuf);
...
//						ftp.current_cmd = NO_CMD;
//    				disconnect(DATA_SOCK);							
//    				size = sprintf((char *)dbuf, "550 File not found \"%s\"\r\n", ftp.filename);
//    				send(CTRL_SOCK, dbuf, size);
//						}

						size=Network_FTP_Answer_RETR_CMD(send, (uint8_t *)dbuf, (uint8_t *)ftp.filename);
						...
						if(size)
						{
							...
							disconnect(DATA_SOCK);
							size = sprintf((char *)dbuf, "226 Successfully transferred \"%s\"\r\n", ftp.filename);
							send(CTRL_SOCK, (uint8_t *)dbuf, size);
						}
						else
						{
							ftp.current_cmd = NO_CMD;
...
//								printf("########## dbuf:%s\r\n", dbuf);
								...
								size=Network_FTP_Save_File(dbuf, ftp.filename, ret);
//								if(size==-1)
//									{
//										while(1){}
//									}
							...
							if(getSn_SR(DATA_SOCK) != SOCK_ESTABLISHED){									
									if(Network_FTP_Save_File_Size(dbuf,ftp.filename)!=-1)
									{
										size = sprintf((char *)dbuf, "226 Successfully transferred \"%s\"\r\n", ftp.filename);
										send(CTRL_SOCK, dbuf, size);
									}
...
char proc_ftpd(char * buf)
{
	...
	if (ftp.state == FTPS_NOT_LOGIN)
	{
		...
		{
			case USER_CMD:
			if(!memcmp((char *)(buf+sizeof("user ")-1),(char *)&NETWORK.FTP.Login,strlen((char *)&NETWORK.FTP.Login))){			
//				slen = sprintf(sendbuf, "331 Password required for %s\r\n",(char *)(buf+sizeof("user ")-1));
//				send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
//				return 0;
			}
			else
			{
				slen = sprintf(sendbuf, "332 Please log in with USER and PASS\r\n");
				send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
				return 0;				
			}		
				break;
			case PASS_CMD:				
//			if(!memcmp((char *)(buf+sizeof("pass ")-1),(char *)&NETWORK.FTP.Password,strlen((char *)&NETWORK.FTP.Password)))
//			{			
//				slen = sprintf(sendbuf, "230 User %s logged in.\r\n",ftp.username);
//				send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
//			}
//			else
//			{
...
//			}
	...
	switch (cmdp - commands)
	{
		...
		case CLNT_CMD:
			//fsprintf(CTRL_SOCK, unimp);
			slen = sprintf(sendbuf, "200 Noted.\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			break;
...
//#####################################################################################					
			case OPTS_CMD:					
				#if defined(_FTP_DEBUG_)
					printf("200 UTF8 set to on\r\n");
				#endif			
				slen = sprintf((char *)sendbuf, "200 UTF8 set to on\r\n");
				send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			break;	
			case NOOP_CMD:					
				#if defined(_FTP_DEBUG_)
					printf("200 NOOP command successful\r\n");
				#endif			
				slen = sprintf((char *)sendbuf, "200 NOOP command successful\r\n");
				send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			break;
			...
			case SIZE_CMD:
				...
				if(slen > 3)
				{
					slen=Network_Check_Size_File(arg);				
...
//				*tmpstr = '/';
		...
		case RETR_CMD :
			...
			if (*arg=='/'){tmpstr++;}
			if(strlen(ftp.workingdir) == 1)
				sprintf(ftp.filename, "/%s", tmpstr);
			else
				sprintf(ftp.filename, "%s/%s", ftp.workingdir, tmpstr);
		...
		case SITE_CMD :
			...
			arg[slen - 2] = 0x00;
		if(!strcmp(arg,"help")){
			slen = sprintf(sendbuf, "214-The following SITE commands are recognized (* =>'s unimplemented)\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "UTIME <sp> YYYYMMDDhhmm[ss] <sp> path\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "SYMLINK <sp> source <sp> destination\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "RMDIR <sp> path\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "The following SITE extensions are recognized:\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "RATIO -- show all ratios in effect\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "HELP\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "CHGRP\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "CHMOD\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
			slen = sprintf(sendbuf, "214 Direct comments to root@%d.%d.%d.%d\r\n", NETWORK.gWIZNETINFO.ip[0],NETWORK.gWIZNETINFO.ip[1],NETWORK.gWIZNETINFO.ip[2],NETWORK.gWIZNETINFO.ip[3]);
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
		}
		else
		{
...
char ftplogin(char * pass)
{
	...
	//fsprintf(CTRL_SOCK, logged);
			...
			if((!memcmp((char *)(ftp.username),(char *)&NETWORK.FTP.Login,strlen((char *)&NETWORK.FTP.Login)))
				&&(!memcmp((char *)(pass),(char *)&NETWORK.FTP.Password,strlen((char *)&NETWORK.FTP.Password))))
					{		
						#if defined(_FTP_DEBUG_)
							printf("%s logged in\r\n", ftp.username);
						#endif						
