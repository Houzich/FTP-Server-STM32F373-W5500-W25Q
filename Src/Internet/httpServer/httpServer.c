...
/*****************************************************/
	...
	{		
		// Data read from external data flash memory
		send_len = Network_HTTP_Read_From_FlashBuf(addr, &buf[0], send_len);
		//send_len = 0;
	}
		...
		case METHOD_GET :
			...
			else
			{
				...
				else
				{
					//----------МОЕ
					HTTPSock_Status[get_seqnum].storage_type = DATAFLASH;
					content_found = Network_HTTP_Check_Response((char *)uri_name, &content_addr, &file_len);
					//
...
uint8_t display_reg_webContent_list(void)
{
	...
	else
	{
		...
		for(i = 0; i < total_content_cnt; i++)
		{
			...
			printf("%d byte, ", web_content[i].content_len);
